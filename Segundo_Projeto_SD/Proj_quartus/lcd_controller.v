module lcd_controller (
    input wire        clk,               
    input wire        reset,             
    input wire        system_on,         // <--- NEW: Connected to the CPU's ON register
    ...
    parameter ST_default     = 4'd0,
              ...
              ST_wait_clr1   = 4'd11,
              ST_wait_clr2   = 4'd12,
              ST_SHUTDOWN    = 4'd13,   // <--- NEW: Sends the Display OFF command
              ST_OFF         = 4'd14;   // <--- NEW: Safe idle state when unprogrammed/off

    // State Transition Logic accounting for System Power Status
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            estado_atual <= ST_OFF;
        end else begin
            if (!system_on) begin
                // If system goes off, force shutdown sequence unless already off
                ...
                // If system is on, leave ST_OFF and process normally
    ...
    always @(*) begin
        ...
        case (estado_atual)
            ...
            ST_wait_clr2: prox_estado = system_on ? ST_avancar : ST_OFF; // Route based on power
        ...
        case (estado_atual)
            ...
            ST_iniciar:  begin LCD_RS = 1'b0; LCD_DATA = 8'h0C; end // Display ON
            ST_clr:      begin LCD_RS = 1'b0; LCD_DATA = 8'h01; end // Clear display
            ST_SHUTDOWN: begin LCD_RS = 1'b0; LCD_DATA = 8'h08; end // Display OFF Command
    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            if (estado_atual != ST_default && 
                ...
                estado_atual != ST_OFF &&            // <--- EN remains low when system is powered off
